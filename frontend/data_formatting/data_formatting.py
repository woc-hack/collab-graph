...
def main(in_path, out_path, min_authors):
    ...
    with io.open(in_path, encoding="latin-1") as f:
        ...
        for line in f:
            try:
                ...
                if (project, project_index) in p2i2a_dict:
                    p2i2a_dict[(project, project_index)].append(author)
                else:
                    p2i2a_dict[(project, project_index)] = [author]
    ...
    print(f"before filtering: {len(p2i2a_dict)}, after filtering: {len(filtered_p2a_dict)}")
